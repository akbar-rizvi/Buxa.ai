...
export default class document {
      ...
      return await postgresdb.transaction(async (tx) => {
        ...
        const newDocument = await postgresdb
          ...
          .returning({
            ...
            isPosted: documents.isPosted,
          }); // returing isPosted also
        const credits = await postgresdb
          .update(users)
          .set({
            credits: sql`${userDetails[0].credits} - 1`,
            usedCredits: sql`${userDetails[0].usedCredits}+1`,
            totalContent: sql`${userDetails[0].totalContent}+1`,
            coc: sql`${userDetails[0].coc}+1`,
          })
          .where(eq(users.id, userId))
  ...
  static getDocumentsByUserId = async (userId: number): Promise<any> => {
    try {
      //sent all data instead of selected data 28/11
      const getDocument = await postgresdb
        ...
        .where(
          and(
            ...
            eq(documents.documentType, "article")
          )
        )
        .execute();
      return getDocument;
  ...
  static getResearchbyUserId = async (userId: number): Promise<any> => {
    try {
      return postgresdb.query.documents.findMany({
        where: and(
          eq(documents.userId, userId),
          eq(documents.documentType, "research"),
          eq(documents.isDeleted, false)
        ),
      });
    } catch (error) {
      ...
      return await postgresdb.transaction(async (tx) => {
        if (id == 0) {
          ...
          const data = await tx
            ...
            //modified in users returning credits with  credits: users.credits and sending in object
          const userdetails = await tx
            .update(users)
            .set({
              credits: sql`${userDetails[0].credits} - 1`,
              usedCredits: sql`${userDetails[0].usedCredits}+1`,
              totalResearch: sql`${userDetails[0].totalResearch}+1`,
              cor: sql`${userDetails[0].cor}+1`,
            })
            .where(eq(users.id, userId))
            .returning({ credits: users.credits })
            .execute();

          //const updatedUserDetails = await tx.select().from(users).where(eq(users.id, userId));
          return { data, credits: userdetails[0].credits };
        } else {
          const userDetails = await tx
            .select()
            .from(users)
          ...
          const userdetails=await tx
            ...
            .where(eq(users.id, userId)).returning({credits:users.credits}).execute();
          return {updatedData,credits:userdetails[0].credits};
        }
      });
    } catch (error) {
      throw new Error(error);
  ...
  static deleteResearch = async (
    ...
    try {
      ...
      return await postgresdb.transaction(async (tx) => {
        ...
        const length = docData.content.filter(
          (item: any) => !item.isDeleted
        ).length;
        if (length == 0) {
          return await tx
            .update(documents)
            .set({ content: docData.content, isDeleted: true })
            .where(
              and(
                eq(documents.id, documentId),
                eq(documents.userId, userId),
